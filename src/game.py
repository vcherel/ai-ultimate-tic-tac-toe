...
class Game:
    def __init__(self):
        player1 = Player(
            team=True, auto=variables.player1_auto, strategy=variables.player1_strategy
        )
        player2 = Player(
            team=False, auto=variables.player2_auto, strategy=variables.player2_strategy
        )
    ...
    def play(self, pos=None):
        ...
        if pos is not None:
            if not board.try_click(pos):
                return  # clicked outside a playable box — don't change player
        else:
            ...
            if box_to_play not in playable_boxes:
                raise ValueError(
                    "Error: The chosen box to play is not in the list of playable boxes"
                )
            else:
